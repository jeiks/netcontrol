...
from netcontrol.utils import detect_platform, shell, shell_status, download
from netcontrol.feedback import *
import netcontrol
...
class NetcontrolVersionRequirementError(BaseRequirementError):
    """
    Exception that means a plugin wasn't loaded due to
    unsupported Netcontrol version
    """
...
class PluginLoader:
    ...
    def register_mgr(mgr):
        """
        Registers an :class:`netcontrol.com.PluginManager` from which the unloaded
    ...
    def load(plugin):
        ...
        from netcontrol import generation, version
        ...
        try:
            ...
            if not 'GENERATION' in mod.__dict__ or mod.GENERATION != generation:
                raise NetcontrolVersionRequirementError('other Netcontrol platform generation')
            ...
            setattr(netcontrol.plugins, plugin, mod)
    ...
    def get_plugin_path(app, id):
        """
        Returns path for plugin's files. Parameters: :class:`netcontrol.core.Application`, ``str``
        """
        if id in PluginLoader.list_plugins():
            return app.config.get('netcontrol', 'plugins')
        else:
...
class RepositoryManager:
    """
    Manages official Netcontrol plugin repository. ``cfg`` is :class:`netcontrol.config.Config`
    ...
    def __init__(self, cfg):
        self.config = cfg
        self.server = cfg.get('netcontrol', 'update_server')
        self.refresh()
    ...
    def update_available(self):
        ...
        """
        try:
            data = eval(open('/var/lib/netcontrol/plugins.list').read())
        except:
            return
        self.available = []
        for item in data:
    ...
    def update_list(self):
        ...
        """
        from netcontrol import generation, version
        if not os.path.exists('/var/lib/netcontrol'):
            os.mkdir('/var/lib/netcontrol')
        send_stats(self.server, PluginLoader.list_plugins().keys())
        data = None
	#download('http://%s/api/plugins?pl=%s&gen=%i' % (self.server,detect_platform(),generation))
        try:
            open('/var/lib/netcontrol/plugins.list', 'w').write(data)
        except:
            pass
        self.update_installed()
        self.update_available()

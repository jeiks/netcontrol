...
from netcontrol.api import get_environment_vars
...
class AuthManager(object):
    ...
    def __init__(self, config, app, dispatcher):
        ...
        self._config = config
        self._enabled = False
        if config.has_option('netcontrol', 'auth_enabled'):
            if config.getint('netcontrol', 'auth_enabled'):
    ...
    def __call__(self, environ, start_response):
            ...
            if self._config.has_option('users', user):
                ...
                if sample == resp:
                    ...
                    start_response('200 OK', [
                        ...
                        ('X-Netcontrol-Auth', 'ok'),
                    ])
                    return ''

            start_response('200 OK', [
                ('Content-type','text/plain'),
                ('X-Netcontrol-Auth', 'fail'),
            ])
            return 'Login failed'
        ...
        start_response('200 OK', [
            ...
            ('X-Netcontrol-Auth', 'start'),
            ('X-Netcontrol-Challenge', challenge),
        ])
        return templ.render()

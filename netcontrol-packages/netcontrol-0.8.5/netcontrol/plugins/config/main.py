...
from netcontrol.utils import hashpw
from netcontrol.plugins.recovery.api import *
...
class ConfigPlugin(CategoryPlugin):
    ...
    def get_ui(self):
        ...
        # General
        ui.find('bind_host').set('value', self.app.gconfig.get('netcontrol', 'bind_host', ''))
        ui.find('bind_port').set('value', self.app.gconfig.get('netcontrol', 'bind_port', ''))
        ui.find('ssl').set('checked', self.app.gconfig.get('netcontrol', 'ssl', '')=='1')
        ui.find('cert_file').set('value', self.app.gconfig.get('netcontrol', 'cert_file', ''))
        ui.find('cert_key').set('value', self.app.gconfig.get('netcontrol', 'cert_key', ''))

        # Security
        ui.find('httpauth').set('checked', self.app.gconfig.get('netcontrol','auth_enabled')=='1')
        tbl = ui.find('accounts')
        for s in self.app.gconfig.options('users'):
            tbl.append(
                    UI.DTR(
                        UI.Label(text=s),
                        UI.TipIcon(text='Excluir', icon='/dl/core/ui/stock/delete.png', id='deluser/'+s),
                    )
                )

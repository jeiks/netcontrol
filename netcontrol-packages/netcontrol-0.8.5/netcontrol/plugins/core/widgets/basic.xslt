    ...
    <xsl:variable name="onclickjs">
        <xsl:choose>
            <xsl:when test="@warning != ''">
                return Netcontrol.showWarning('<xsl:value-of select="@warning"/>',
                    '<xsl:value-of select="@id"/>');
            </xsl:when>

            <xsl:when test="@onclick = 'form'">
                return Netcontrol.submit('<xsl:value-of select="@form" />',
                    '<xsl:value-of select="@action" />');
            </xsl:when>

            <xsl:when test="@onclick = '' or not (@onclick)">
                return Netcontrol.query('/handle/<xsl:value-of
...
<xsl:template match="linklabel">
    <a href="#" onclick="javascript:return Netcontrol.query('/handle/linklabel/click/{@id}');" class="ui-el-link" style="{x:iif(@bold, 'font-weight: bold;', '')}">
        <xsl:value-of select="@text" />
    </a>
</xsl:template>

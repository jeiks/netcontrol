    ...
    <li class="ui-el-tab-header {x:iif(@active or (../@active = @id), 'active', '')}">
        <xsl:if test="../@live = 'True' or @live = 'True'">
            <xsl:attribute name="onclick">
                <xsl:if test="@form">
                    return Netcontrol.submit('<xsl:value-of select="@form" />', '<xsl:value-of select="@id" />');
                </xsl:if>
                <xsl:if test="not(@form)">
                    return Netcontrol.query('/handle/tab/click/<xsl:value-of select="@id" />');
                </xsl:if>
            </xsl:attribute>
        </xsl:if>
        <a href="#{@id}">
...
<xsl:template match="treecontainer">
    <div class="ui-el-treecontainernode">
        <a href="#" onclick="return Netcontrol.UI.toggleTreeNode('{@id}');" class="text">
...
<xsl:template match="listitem">
    <li class="{x:iif(@active, 'active', '')}" onclick="return Netcontrol.query('/handle/listitem/click/{@id}');">
        <xsl:apply-templates />
    </li>
</xsl:template>
...
<xsl:template match="editable">
    <a href="#" onclick="return Netcontrol.UI.editableActivate('{x:idesc(@id)}')" class="ui-el-editable-inactive" id="{x:idesc(@id)}-normal">
    ...
    <div id="{x:idesc(@id)}" class="ui-el-editable input-append" style="display:none">
        ...
        <hlabel class="add-on active">
            <img href="#" src="/dl/core/ui/stock/dialog-ok.png" onclick="return Netcontrol.submit('{x:idesc(@id)}', 'OK')" />
        </hlabel>
    </div>
</xsl:template>

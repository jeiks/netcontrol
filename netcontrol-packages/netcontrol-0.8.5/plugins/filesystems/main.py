...
class FSPlugin(CategoryPlugin):
    ...
    def get_ui(self):
        ...
        for u in self.fstab:
            t.append(UI.DTR(
                    ...
                    UI.HContainer(
                        UI.TipIcon(icon='/dl/core/ui/stock/edit.png', id='edit/'+str(self.fstab.index(u)), text='Editar'),
                        UI.TipIcon(icon='/dl/core/ui/stock/delete.png', id='del/'+str(self.fstab.index(u)), text='Excluir', warning='Remove %s from fstab'%u.src)
                    ),
                ))
    ...
    def get_ui_sources_list(self, e):
        ...
        cst &= e.src != 'proc'
        lst.append(UI.SelectOption(text='Customizar', value='custom', selected=cst))
        return lst, cst

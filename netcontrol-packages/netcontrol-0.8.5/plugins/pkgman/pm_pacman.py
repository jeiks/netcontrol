...
class PacmanPackageManager(Plugin):
    ...
    def get_lists(self):
        utils.shell_bg('pacman -Sy', output='/tmp/netcontrol-pacman-output', deleteout=True)
    ...
    def apply(self, st):
        ...
        utils.shell_bg(fcmd, output='/tmp/netcontrol-pacman-output', deleteout=True)
    ...
    def get_busy_status(self):
        try:
            return open('/tmp/netcontrol-pacman-output', 'r').read().splitlines()[-1]
        except:
            return ''
    ...
    def abort(self):
        ...
        utils.shell('rm /tmp/netcontrol-pacman-output')
    ...
    def _save_pending(self, p):
        f = open('/tmp/netcontrol-pacman-pending.list', 'w')
        for x in p:
            f.write('%s %s\n' % (p[x], x))
        f.close()

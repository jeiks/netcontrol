...
class PortsPackageManager(Plugin):
    ...
    def refresh(self, st):
        ...
        st.pending = {}
        try:
            ss = open('/tmp/netcontrol-ports-pending.list', 'r').read().splitlines()
            for s in ss:
                s = s.split()
                try:
                    st.pending[s[1]] = s[0]
    ...
    def get_lists(self):
        utils.shell_bg('portsnap fetch', output='/tmp/netcontrol-ports-output', deleteout=True)
    ...
    def apply(self, st):
        ...
        for x in st.pending:
            if st.pending[x] == 'install':
                ...
                cmd2 += ' ' + x
        utils.shell_bg('%s; %s'%(cmd,cmd2), output='/tmp/netcontrol-ports-output', deleteout=True)

    def is_busy(self):
        return os.path.exists('/tmp/netcontrol-ports-output')

    def get_busy_status(self):
        try:
            return open('/tmp/netcontrol-ports-output', 'r').read().splitlines()[-1]
        except:
            return ''
    ...
    def abort(self):
        ...
        utils.shell('rm /tmp/netcontrol-ports-output')
    ...
    def _save_pending(self, p):
        f = open('/tmp/netcontrol-ports-pending.list', 'w')
        for x in p:
            f.write('%s %s\n' % (p[x], x))
        f.close()

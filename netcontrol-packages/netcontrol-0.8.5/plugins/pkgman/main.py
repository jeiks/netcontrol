                ...
                r = UI.DTR(
                        ...
                        UI.Label(text=p.description),
                            UI.HContainer(
                                ...
                                UI.TipIcon(icon='/dl/pkgman/package-available.png', text='Desmarcar', id='cancel/'+p.name)
                                    if p.name in self._status.pending else
                                UI.TipIcon(icon='/dl/pkgman/package-upgrade.png', text='Selecionar', id='upgrade/'+p.name),
                                spacing=0
                            ),
                    )
                tbl_pkgs.append(r)
            ...
            for p in sorted(self._status.full.keys()):
                ...
                r = UI.DTR(
                        ...
                        UI.Label(text=p.description),
                            UI.HContainer(
                                UI.TipIcon(icon='/dl/core/ui/stock/info.png', text='Info', id='info/'+p.name),
                                UI.TipIcon(icon='/dl/pkgman/package-install.png', text='Reinstalar', id='install/'+p.name),
                                UI.TipIcon(icon='/dl/pkgman/package-remove.png', text='Remover', id='remove/'+p.name),
                                spacing=0
                            ),
                    )
                tbl_pkgs.append(r)
        ...
        if self._current == 'search':
                ...
                r = UI.DTR(
                        ...
                        UI.Label(text=self._search[p].description),
                            UI.HContainer(
                                ...
                                UI.TipIcon(icon='/dl/pkgman/package-install.png', text='Instalar', id='install/'+p) if self._search[p].state == 'removed' else
                                UI.TipIcon(icon='/dl/pkgman/package-remove.png', text='Remover', id='remove/'+p),
                                spacing=0
                            ),
                )
                tbl_pkgs.append(r)
            if len(self._search.keys()) > 50:
                tbl_pkgs.append(UI.DTR(
                    UI.DTD(
                        UI.Label(text='Pacotes demais. Tente usar uma consulta mais precisa'),
                        colspan=5
                    )
                ))
            ...
            for p in sorted(self._status.pending.keys()):
                r = UI.DTR(
                        ...
                        UI.Label(),
                            UI.HContainer(
                                UI.TipIcon(icon='/dl/core/ui/stock/info.png', text='Info', id='info/'+p),
                                UI.TipIcon(icon='/dl/core/ui/stock/delete.png', text='Cancelar', id='cancel/'+p),
                                spacing=0
                            ),
                    )
                tbl_pkgs.append(r)
    ...
    def get_ui_info(self):
        ...
        ui = UI.LT(
                UI.LTR(
                    ...
                    UI.Label(text='Pacote:', bold=True),
                    UI.Label(text=pkg, bold=True)
                ),
                UI.LTR(
                    UI.Label(text='Instalado:'),
                    UI.Label(text=info.installed)
                ),
                UI.LTR(
                    UI.Label(text='Disponível:'),
                    UI.Label(text=info.available)
                ),
                UI.LTR(
                    UI.Label(text='Descrição:'),
                    UI.Container(
                ...
                UI.LTR(
                    UI.LTD(
                        UI.HContainer(
                            UI.Button(text='(Re)instalar', id='install/'+pkg),
                            UI.Button(text='Remover', id='remove/'+pkg)
                        ),
                        colspan=2
                    )
                )

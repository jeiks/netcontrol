...
class APTPackageManager(Plugin):
    ...
    def get_lists(self):
        utils.shell_bg('apt-get update', output='/tmp/netcontrol-apt-output', deleteout=True)
    ...
    def apply(self, st):
        ...
        utils.shell_bg(cmd, output='/tmp/netcontrolnetcontrol-apt-output', deleteout=True)
    ...
    def get_busy_status(self):
        try:
            return open('/tmp/netcontrol-apt-output', 'r').read().splitlines()[-1]
        except:
            return ''
    ...
    def _save_pending(self, p):
        f = open('/tmp/netcontrol-apt-pending.list', 'w')
        for x in p:
            f.write('%s %s\n' % (p[x], x))
        f.close()

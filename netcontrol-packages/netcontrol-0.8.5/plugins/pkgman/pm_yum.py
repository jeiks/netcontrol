...
class YumPackageManager(Plugin):
    ...
    def refresh(self, st):
        ...
        st.pending = {}
        try:
            ss = open('/tmp/netcontrol-yum-pending.list', 'r').read().splitlines()
            for s in ss:
                s = s.split()
                try:
                    st.pending[s[1]] = s[0]
    ...
    def get_lists(self):
        utils.shell_bg('yum check-update', output='/tmp/netcontrol-yum-output', deleteout=True)
    ...
    def apply(self, st):
        ...
        utils.shell_bg(cmd, output='/tmp/netcontrolnetcontrol-yum-output', deleteout=True)
    ...
    def get_busy_status(self):
        try:
            return open('/tmp/netcontrol-yum-output', 'r').read().splitlines()[-1]
        except:
            return ''
    ...
    def abort(self):
        ...
        utils.shell('rm /tmp/netcontrol-yum-output')
    ...
    def _save_pending(self, p):
        f = open('/tmp/netcontrol-yum-pending.list', 'w')
        for x in p:
            f.write('%s %s\n' % (p[x], x))
        f.close()
